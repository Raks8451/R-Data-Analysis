...
temp <- ggplot(data = diamonds,aes(x=carat,y=price,col = clarity)) +
  geom_smooth()
#temp + geom_text(data = diamonds[diamonds$carat == "2",], aes(label = clarity), hjust = 0.7, vjust = 1)

#install.packages("directlabels")
library(directlabels)
direct.label(temp,list(last.points, hjust = .5,
                       vjust = .5))
...
ggplot(mtcars,aes(x=mpg)) + 
  geom_histogram(binwidth = 3,fill = mycolor)
...
ggplot(economics, aes(x=date,y=unemploy/pop,col = psavert)) +
  geom_line()
...
temp + scale_color_continuous(name="",
                              breaks = c(3000, 10000, 15000),
                              labels = c("3000", "'10000", "'15000"))
